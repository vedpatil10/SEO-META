...
function parseGid(sheetUrl) {
  const match = String(sheetUrl || '').match(/[#&]gid=([0-9]+)/);
  return match ? match[1] : '';
}
...
async function fetchSheetRowsViaApi(spreadsheetId, sheetName, accessToken) {
...
async function fetchSheetRows(spreadsheetUrl, sheetName, googleAccessToken) {
  ...
  if (googleAccessToken) {
    const { rows, headers } = await fetchSheetRowsViaApi(spreadsheetId, sheetName, googleAccessToken);
    return {
      spreadsheetId,
      gid,
      csvUrl: null,
  ...
  if (hasGoogleServiceAccount()) {
    ...
    const { rows, headers } = await fetchSheetRowsViaApi(spreadsheetId, sheetName, accessToken);
    return {
      spreadsheetId,
      gid,
      csvUrl: null,
  ...
  for (const candidate of candidates) {
    try {
      ...
      return { spreadsheetId, gid, csvUrl: candidate, rows, headers, source: 'csv-export' };
    } catch (error) {
      lastError = error;
    }
  }
...
async function callAnthropic(apiKey, prompt, maxTokens) {
  ...
  const minIntervalMs = Number(process.env.ANTHROPIC_MIN_INTERVAL_MS || 13000);
  const maxAttempts = Number(process.env.ANTHROPIC_MAX_RETRIES || 4);
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const retryAfterMs = retryAfterHeader ? Number(retryAfterHeader) * 1000 : attempt * 15000;
    await sleep(Number.isFinite(retryAfterMs) ? retryAfterMs : 15000);
  }
...
Extract and return ONLY valid JSON:
{
  ...
  const raw = await callAnthropic(anthropicKey, prompt, 500);
...
`;
  ...
  const raw = await callAnthropic(anthropicKey, prompt, 300);
...
{"meta_title":"","meta_description":""}`;
  ...
  for (let attempt = 1; attempt <= maxGenerateAttempts; attempt += 1) {
    ...
    const raw = await callAnthropic(keys.anthropicKey, buildPrompt(extraInstruction), 700);
...
async function runSeoAgent({ spreadsheetUrl, sheetName, googleAccessToken, onProgress }) {
  ...
  const { rows, headers, spreadsheetId, csvUrl, source } = await fetchSheetRows(spreadsheetUrl, sheetName, googleAccessToken);
  ...
  if (generatedRows.length && (source === 'google-api' || source === 'google-oauth')) {
    ...
    sheetWrite = await writeRowsViaApi(spreadsheetId, sheetName, headers, generatedRows, accessToken);
  }
